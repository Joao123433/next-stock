...
import UseUpdateItems from "@/hooks/UseUpdateItem";
import { useRouter } from "next/dist/client/router";
import Link from "next/link";

export default function UpdateItems() {
  const itemId = useRouter().query.UpdateItem as string
      ...
      <section className="flex flex-col items-center gap-5 h-full">
        ...
        <div className="w-5/6 flex justify-start">
          <button className="h-12 hover:border-blue-500" onClick={handleClick}>
            <Link href="/items/Items" className="text-white text-lg px-5 py-3">Atualizar</Link>
          </button>
        </div>
    </section>
    </ItemsLayout>
